...
import { accessTicketValidation,listaccessTicketValidation, ticketAccessIdValidation, updateTicketAccesValidation } from "../validators/ticket-validator";
import { generateValidationErrorMessage } from '../validators/generate-validation-message';
import { AppDataSource } from '../../database/database';
import { TicketShowtimeAccesses } from "../../database/entities/ticketShowtimeAccesses";
import { TicketAccesUsecase } from '../../domain/TicketAccesses-usecase';
export const ticketAccessesHandler = (app: express.Express) => {
  
    app.get("/ticketShowtimeAccesses", async (req: Request, res: Response) => {
        try {
            // Validation de la requête
            const validation = listaccessTicketValidation.validate(req.query);
            ...
            const listAcessTicketRequest = validation.value;
            let limit = 20;
            if (listAcessTicketRequest.limit) {
                limit = listAcessTicketRequest.limit;
                console.log('Limite:', limit);
            }
            const page = listAcessTicketRequest.page ?? 1;
          
            const ticketAccessUsecase = new TicketAccesUsecase(AppDataSource);
      
       
            const listTickets = await ticketAccessUsecase.listTicketsAcces({ ...listAcessTicketRequest, page, limit });
